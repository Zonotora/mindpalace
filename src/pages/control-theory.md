...
## Routh-Hurwitz
Denna teknik är bra att använda om inte $ 1 + L(s) $ lätt kan faktoriseras eller om polynomet innehåller fria variabler.
...
### Amplitudmarginal
Betecknas $ A_m $ och anger hur mycket variation förstärkningen kan ha i kretsöverföringen innan det återkopplade systemet blir instabilt.
$$
\boxed{A_m = -\frac{1}{L(j\omega_\pi)} = \frac{1}{ | L(j\omega_\pi) |}}
$$
$$
A_{m_{dB}} = 20 \log_{10} A_m
$$
där $ \omega_\pi $ är fasen då $ \angle L(j\omega_\pi) = -180 \degree $. Amplitudmarginalen kan läsas direkt ur ett Bodediagram och läses då som ett avstånd.

### Fasmarginal
Betecknas $ \varphi_m $ och anger hur mycket fasvidring som tillåts i kretsöverföringen innan det återkopplade systemet blir instabilt. Fasmarginalen ger upphov till
$$
| L(j\omega_c) | = 1
$$
genom
$$
e^{-j\varphi_m} L(j\omega_c) = -1
$$
...
---
Följande relation gäller mellan en reell tidskontinuerlig pol $ s = -a $ och motsvarande tidsdiskret reell pol $ z = e^{-ah} $
$$
\boxed{z = e^{sh}}
$$
...
$$
G(s) = \frac{a}{s + a} \qquad G_d(z) = \frac{(1-e^{-ah}) z^{-1}}{1-e^{-ah} z^{-1}}
$$
...
1. Gör om överföringsfunktion $ G(s) $ till differentialekvation och skriv på tillståndsform
$$
\begin{aligned}
\dot x(t) &= Ax(t) + Bu(t)\\
\\
...
$$
\begin{aligned}
zX(z) - x(0) &= A_dX(z) + B_dU(z) \\
Y(z) &= CX(z) + DU(z)
\end{aligned}
$$
5. Överföringsfunktionen $ G_d(z) $
$$
